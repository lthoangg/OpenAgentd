...
For simulator development, `http://localhost:8000` usually reaches the Mac backend. Physical devices should use a LAN IP or HTTPS endpoint. SSE reconnects retain the database-before-stream ordering and back off from 1.5 seconds to a 30-second ceiling while offline, resetting as soon as an event arrives. Mobile does not initialize the desktop updater; releases are delivered through the platform distribution channel.
