...
The root route stores/restores the last route on a best-effort basis. Updater dismissal state and desktop access-key persistence use the same pattern. `localStorage` failures are swallowed so restricted WebViews, private browsing, or mobile shells with disabled storage still boot into the app instead of crashing.
