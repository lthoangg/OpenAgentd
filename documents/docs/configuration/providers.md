...
When routed to `/v1/responses`, `max_tokens` maps to `max_output_tokens`. Sampling controls (`temperature`/`top_p`) are not sent by any provider — see [`agents.md`](./agents.md#frontmatter-fields).
...
**Thinking block round-trip.** When extended thinking is active the API may return two block types that must be echoed verbatim in subsequent turns (omitting or modifying either causes HTTP 400): `thinking` blocks (carry summarised text + an opaque `signature`) and `redacted_thinking` blocks (safety-redacted reasoning, carry only an opaque `data` field). Both are stored in `SessionMessage.extra` and restored on load. See `app/agent/providers/anthropic/anthropic.py` for the full contract.
...
| `openai` | `service_tier` | `"fast"` ➔ `"priority"` | Opts into OpenAI's Priority latency processing tier. Only sent for official `api.openai.com` requests. |
