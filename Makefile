...
prompt-budget: ## Count system prompt, tool schema, and bundled skill tokens
	@tmp=$$(mktemp -d); TMP_AGENTS=$$tmp uv run python -c 'import os; from pathlib import Path; from app.agent.loader import ensure_builtin_openagentd_lead; p=Path(os.environ["TMP_AGENTS"]); ensure_builtin_openagentd_lead(p, mode="normal")'; uv run python -m manual.inspect_prompt --dir $$tmp --date 2026-01-01 --skills-scope builtin --stats-only; status=$$?; rm -rf $$tmp; exit $$status

prompt-budget-json: ## Same as prompt-budget but emit stable JSON for tracking/CI
	@tmp=$$(mktemp -d); TMP_AGENTS=$$tmp uv run python -c 'import os; from pathlib import Path; from app.agent.loader import ensure_builtin_openagentd_lead; p=Path(os.environ["TMP_AGENTS"]); ensure_builtin_openagentd_lead(p, mode="normal")'; uv run python -m manual.inspect_prompt --dir $$tmp --date 2026-01-01 --skills-scope builtin --stats-only --json; status=$$?; rm -rf $$tmp; exit $$status
