...
@dataclass(frozen=True)
class VoiceConfig:
    """Resolved config for the ``voice`` section of ``multimodal.yaml``.
...
def get_voice_config() -> VoiceConfig | None:
    ...
    if not isinstance(enabled, bool):
        logger.warning("multimodal_voice_enabled_invalid value={}", enabled)
        return None
    if not enabled:
        return None
    ...
    if not isinstance(model_str, str) or ":" not in model_str:
        logger.warning("multimodal_voice_model_invalid model={}", model_str)
        return None
