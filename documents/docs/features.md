...
**Latest release:** v1.102.0 · July 13, 2026 · [release notes](https://github.com/lthoangg/openagentd/releases/tag/v1.102.0)
...
- **LAN access key for external clients** `[v1.43.0]` — `openagentd start --lan --key`
  stores a `settings.yaml` bearer key so desktop, mobile, and browser clients need the key before controlling a LAN-exposed server. OpenAgentd-managed launchers refuse non-loopback binds without a configured key `[v1.101.0]`. See [`cli.md`](./cli.md) and [ADR-0002](../adrs/0002-require-authentication-for-non-loopback-bindings.md).
