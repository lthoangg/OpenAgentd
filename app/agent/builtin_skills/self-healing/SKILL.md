...
| `tools` | Extra tools on top of built-in profile: `web_search`, `web_fetch`, `read`, `patch`, `grep`, `glob`, `shell`, `bg`, `generate_image`, `generate_video`, plus `<server>_<tool>` for MCP. Never add `skill` or `team_message` — injected automatically. |
