...
import { inferErrorTitle, useTeamStore } from "@/stores/useTeamStore";
...
describe("team error toast subscriber", () => {
  ...
  it("infers intelligent toast titles for action and stream failures", () => {
    ...
    useTeamStore.setState({ error: null });
    useTeamStore.setState({ error: "Cannot undo while agents are working — /stop first" });
...
describe("_handleSSEEvent structured error handling", () => {
  it("parses structured error events into store state", () => {
    useTeamStore.setState({ error: null, isTeamWorking: true });

    useTeamStore.getState()._handleSSEEvent("error", {
  ...
  it("appends provider error blocks directly to blocks and preserves them", () => {
    ...
    useTeamStore.getState()._handleSSEEvent("done", {});
    expect(useTeamStore.getState().agentStreams["lead"].blocks.length).toBe(1);
  });
});
