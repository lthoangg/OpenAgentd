...
- **Hold-to-act (mobile) / right-click (desktop):** Long-pressing a changed-file row or right-clicking it opens an action menu with: Open file, Copy file path, and Discard changes. Discarding shows a confirmation dialog — modified/deleted files run `git checkout -- <path>`; added/untracked files are deleted from disk. The diff view refreshes automatically after a successful discard. Commit rows expose: **Undo commit** (soft-resets the last commit, keeping all changes staged in the working copy), **Revert commit** (creates a new commit reverting the changes of the selected commit, with auto-abort protection on conflicts), Copy short SHA, Copy full SHA, and Copy commit message. A toast appears briefly after any action. Commit subjects are safely URL-decoded before display and before copying, falling back to the raw subject if the string contains a literal `%` character. Haptics fire on hold trigger (medium) and on each action selection (soft).
