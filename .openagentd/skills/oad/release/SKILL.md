...
- Update **all eight release-facing version files** in lockstep, and include the Rust lockfiles in the release commit:
  ...
  - `desktop/src-tauri/tauri.conf.json` — drives **desktop artefact filenames** (`OpenAgentd_<ver>_*.dmg`, `.msi`, `.deb`, `.AppImage`) and the About-panel version in the desktop app.
  - `desktop/src-tauri/Cargo.toml` — `env!("CARGO_PKG_VERSION")` in `main.rs` reads from here for the desktop About panel; keep it equal to `tauri.conf.json` or the version-consistency check fails.
  - `mobile/src-tauri/tauri.conf.json` — drives mobile bundle metadata and is checked against `app/version.txt`.
  - `mobile/src-tauri/Cargo.toml` — mobile Rust package version; keep it equal to `mobile/src-tauri/tauri.conf.json` or the version-consistency check fails.
  - `desktop/src-tauri/Cargo.lock` — **must** be refreshed/committed when the desktop package version changes because `tauri.yml` runs `cargo check --locked` on push.
  - `mobile/src-tauri/Cargo.lock` — **must** be refreshed/committed when the mobile package version changes because `tauri.yml` runs `cargo check --locked` on push.
- **Why this matters:** `app/version.txt` drives the *tag name* both release workflows use (`v<X.Y.Z>`), but bundled artefacts and app metadata read from the Tauri/Cargo files. If you bump only the Python/web/desktop files, CI fails because mobile remains on the prior version; if you bump only `app/version.txt`, you can ship a tag whose downloadable artefacts carry stale filenames or About-panel metadata. If you bump the Rust package versions without updating `desktop/src-tauri/Cargo.lock` and `mobile/src-tauri/Cargo.lock`, the `tauri.yml` push workflow fails immediately at `cargo check --locked` for both apps. Treat the Rust lockfiles as mandatory release files, not optional/generated extras.
...
- Before pushing the version bump, verify the lockfiles actually changed to the target version:

```bash
grep -n 'version = "<version>"' desktop/src-tauri/Cargo.lock mobile/src-tauri/Cargo.lock
```
