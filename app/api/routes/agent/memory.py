...
async def get_tree() -> MemoryTreeResponse:
    """Reconcile and list memory pages with lightweight metadata."""
...
async def search_files(
    ...
    """Deterministic search across global memory."""
    target_scope = resolve_memory_scope()
    manager = get_memory_manager()
    results = await manager.search(target_scope, query)
...
async def lint_files() -> MemoryLintReport:
    """Run deterministic linter across global memory."""
    target_scope = resolve_memory_scope()
    manager = get_memory_manager()
    findings = list(await manager.lint(target_scope))
