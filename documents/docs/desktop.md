...
The tray status starts at `Status: Starting`, changes to `Status: Running` once the bundled sidecar is healthy, and changes to `Status: Error` when no sidecar or saved server is reachable. During desktop frontend reloads, React now waits for bundled-backend readiness before treating the shell as bootstrapped, while external-backend windows skip that wait and restore their saved server immediately. When the user switches a window to a different external server from the backend connection dialog, that window also performs a frontend reload so stale in-memory state is discarded before the UI resumes against the new backend. In the error state the window still opens and the home page shows **Backend unreachable** so the user can choose the builtin sidecar or a saved server from the backend connection dialog.
...
Rows are inserted/removed in place on the pre-built `Submenu` handle (`AppState::usage_submenu` / `usage_rows`) rather than rebuilding the whole tray menu — and only when the rendered rows actually *changed* since the last poll (`AppState::usage_rendered_rows` diff); an unchanged snapshot updates just the footer timestamp, skipping the native-menu churn entirely. Row layout is compact and grouped: a provider with a single limit window renders one flat row leading with the provider label (`🟢 GitHub Copilot · Premium requests · 0% ░░░░░░░░░░ · resets 2h 14m`); a provider with multiple limits renders a header row carrying the *worst* limit's 🟢 (`<70%`) / 🟠 (`70–89%`) / 🔴 (`≥90%`) glyph, followed by indented per-limit rows that don't repeat the provider label, capped at 3 with an indented `… N more` overflow row. Every measurable row also carries a 10-segment block-character meter (`usage::render_bar`, `█` filled / `░` empty) between the percent and the reset countdown — a plain-text stand-in for a real progress bar, since native `MenuItem`s can't render custom widgets (mirrors the filled-bar look of menu-bar apps like [CodexBar](https://github.com/steipete/CodexBar) as closely as a text-only tray menu allows). Reset countdowns are relative (`resets 2h 14m`) — deliberately not wall-clock, since the tray has no reliable timezone source. Rows backed by a backend-substituted last-known-good payload carry a compact `(old)` marker on the provider line. **Refresh Usage Now** forces a live re-check bypassing the backend cache; **Manage Providers…** opens Settings straight to the Providers tab (`settings_providers` desktop command → `openSettings('providers')`).
