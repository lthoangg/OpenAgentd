...
function AgentMessageResult({ result }: { result: string }) {
  const isError =
    result.startsWith('Failed') ||
    result.startsWith('Error') ||
    result.startsWith('Invalid') ||
    result.startsWith('Target agent')
...
function AgentJsonResult({ result }: { result: string }) {
  ...
  return (
    <ul className="space-y-1">
      {entries.map(([key, val]) => {
        const isError = key === 'error' || (key === 'status' && String(val).includes('failed'))
        const displayVal = typeof val === 'object' ? JSON.stringify(val) : String(val)
        return (
          <li key={key} className="flex min-w-0 gap-2 font-mono text-[11px] leading-relaxed">
...
function ToolResultInner({ toolName, operation, result, headerAction, onCollapse }: { toolName: string; operation?: string; result: string; headerAction?: ReactNode; onCollapse?: () => void }) {
  ...
  if (toolName === 'agent_spawn' || toolName === 'agent_merge' || toolName === 'agent_list') {
    return <AgentJsonResult result={result} />
  }
  if (toolName === 'agent_send' || toolName === 'agent_stop') {
    return <AgentMessageResult result={result} />
  }
