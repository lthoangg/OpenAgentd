...
- **Hold-to-act (mobile) / right-click (desktop):** Long-pressing a changed-file row or right-clicking it opens an action menu with: Open file, Copy file path, and Discard changes. Discarding shows a confirmation dialog — modified/deleted files run `git checkout -- <path>`; added/untracked files are deleted from disk. The diff view refreshes automatically after a successful discard. Commit rows expose: Copy short SHA, Copy full SHA, and Copy commit message. A "copied" toast appears briefly after any copy action. Commit subjects are URL-decoded before display and before copying. Haptics fire on hold trigger (medium) and on each action selection (soft).
