...
def scenario_a(base: str) -> bool:
    """Explicit upload is persisted and attached to a queued row."""
    print("\n── Scenario A: explicit upload queued then dequeued ──")

    print(f"  sending slow initial prompt...")
    first = _post_message(base, SLOW_PROMPT)
    ...
    results: list[bool] = []
    results.append(_check("queue POST returns status=queued", queued.get("status") == "queued"))
    ...
    counts = {e["event"]: counts.get(e["event"], 0) + 1 for counts in [{}] for e in events}  # noqa
    counts = {}
    for e in events:
    ...
    if any(e["event"] == "error" for e in events):
        ...
        return False
    results.append(_check("stream completed with done", any(e["event"] == "done" for e in events)))

    history = _get_history_messages(base, session_id)
    queued_row = next((m for m in history if str(m.get("id")) == message_id), None)
    results.append(_check("queued message row present in history", queued_row is not None))
    if queued_row is None:
        return False

    extra = queued_row.get("extra") or {}
    atts = extra.get("attachments") or []
    results.append(_check("queued row has attachments in extra", len(atts) > 0, f"got {len(atts)}"))
    if not atts:
        return all(results)
...
def scenario_b(base: str) -> bool:
    ...
    print(f"  second DELETE must be 404...")
    status2 = _delete_queued(base, session_id, message_id)
    results.append(_check("second DELETE returns 404", status2 == 404, f"got {status2}"))
...
def main() -> int:
    ...
    if args.scenario in ("a", "both"):
        passed.append(scenario_a(base))

    if args.scenario in ("b", "both"):
        passed.append(scenario_b(base))
