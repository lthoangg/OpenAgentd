...
function normalize(form: SandboxForm): SandboxForm {
...
export function SandboxSettingsPage() {
  ...
  const draft = useSettingsDraft<SandboxForm>({
  ...
  return (
    ...
    >
      <SettingsSection title="Denied patterns">
        {patterns.length === 0 ? (
          // Uses the shared EmptyState primitive rather than a hand-rolled
          // dashed box, so this pane matches every other blank state.
          <EmptyState
            fill={false}
            icon={Shield}
            title="No patterns"
