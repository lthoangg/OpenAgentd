...
def transition_instruction(mode: InteractionMode) -> str:
    ...
    if mode == "plan":
        return (
            ...
            "`shell`, `web_fetch`, `web_search`). Shell commands must be strictly read-only "
            "(e.g. `git status`, `git log`, `git diff`, dry runs). Do not edit, patch, or delete files, "
            "and do not execute mutating shell commands, git commits, or package installations.\n"
    ...
    return (
        ...
        "For multi-step work, use `todo_manage` to keep the active milestone visible to the user.\n"
