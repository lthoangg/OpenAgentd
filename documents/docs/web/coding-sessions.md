...
- **Hold-to-act (mobile) / right-click (desktop):** Long-pressing a changed-file or commit row on mobile opens a touch-friendly action sheet, while right-clicking it on desktop opens a native-feeling context menu directly at the cursor coordinates. Discarding shows a confirmation dialog — which on desktop uses responsive side-by-side buttons instead of a stretched vertical layout. Discarding modified/deleted files run `git checkout -- <path>`; added/untracked files are deleted from disk. The diff view refreshes automatically after a successful discard. Commit rows expose: **Undo commit** (soft-resets the last commit, keeping all changes staged in the working copy), **Revert commit** (creates a new commit reverting the changes of the selected commit, with auto-abort protection on conflicts), Copy short SHA, Copy full SHA, and Copy commit message. A toast appears briefly after any action. Commit subjects are safely URL-decoded before display and before copying, falling back to the raw subject if the string contains a literal `%` character. Haptics fire on hold trigger (medium) and on each action selection (soft).
...
- The **Commits** sub-tab surfaces branch divergence from the tracked upstream directly in the tab chrome: `↑` for local commits ahead of origin and `↓` for remote commits behind origin. Both badges come from `GET /api/team/workspace/status`, so they are ready before the history query runs; if the branch has no upstream, neither badge is shown.
