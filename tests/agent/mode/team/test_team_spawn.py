...
- spawn() auto-suffixes (executor → executor#1, executor → executor#2).
...
- Legacy bare-name session adoption — first ``executor#1`` spawn under a lead
  whose DB has a legacy ``executor`` row inherits that row.
...
class TestLoaderLeadOnly:
    ...
    def test_lead_built_members_are_blueprints(self, tmp_path):
        team = _build_dynamic_team(
            tmp_path,
            {"executor": {"description": "writes files"}, "explorer": None},
        )
...
class TestSpawn:
    async def test_first_spawn_returns_hash_one(self, tmp_path):
        ...
        try:
            ...
            assert member.name == "executor#1"
            assert "executor#1" in team.members
            assert "executor#1" in team.mailbox.registered_agents
        finally:
            await team.stop()
    ...
    async def test_auto_suffix_for_parallel_instances(self, tmp_path):
        ...
        try:
            ...
            m3 = await team.spawn("executor")
            assert m1.name == "executor#1"
    ...
    async def test_explicit_instance_id_skips_auto_counter(self, tmp_path):
        ...
        await team.start()
        try:
            m_explicit = await team.spawn("executor", instance_id=5)
            assert m_explicit.name == "executor#5"
            # Next auto-spawn should be #6, not #2.
            m_next = await team.spawn("executor")
            assert m_next.name == "executor#6"
        finally:
            await team.stop()
    ...
    async def test_spawn_with_existing_handle_raises(self, tmp_path):
        ...
        await team.start()
        try:
            await team.spawn("executor", instance_id=1)
    ...
    async def test_negative_instance_id_rejected(self, tmp_path):
        ...
        try:
            with pytest.raises(ValueError, match="must be >= 1"):
                await team.spawn("executor", instance_id=0)
        finally:
            await team.stop()
...
class TestDismiss:
    async def test_dismiss_returns_true_and_removes_from_roster(self, tmp_path):
        ...
        try:
            ...
            assert "executor#1" in team.members
            ok = await team.dismiss("executor#1")
            assert ok is True
            assert "executor#1" not in team.members
            assert "executor#1" not in team.mailbox.registered_agents
        finally:
            await team.stop()
...
class TestCounter:
    async def test_counter_resets_on_new_lead_session(self, tmp_path):
        ...
        try:
            ...
            m = await team.spawn("executor")
            assert m.name == "executor#1"
        finally:
            await team.stop()
    ...
    async def test_counter_survives_restart(self, tmp_path):
        ...
        team1 = _build_dynamic_team(tmp_path, {"executor": None})
        await team1.start()
        sid = str(uuid.uuid7())
        try:
            ...
            for n in (1, 2, 3):
                await team1.dismiss(f"executor#{n}")
        finally:
            await team1.stop()

        # Fresh team — same agents dir, same DB.
        team2 = _build_dynamic_team(tmp_path, {"executor": None})
        await team2.start()
        try:
...
class TestLegacyAdoption:
    async def test_first_spawn_adopts_legacy_bare_name_session(self, tmp_path):
        ...
        try:
            ...
            # adopt the legacy row as ``executor#1``.
            m = await team.spawn("executor")
            assert m.name == "executor#1"
...
class TestRecipientResolution:
    def test_resolve_bare_name_when_unique(self, tmp_path):
        ...
        agent = Agent(name="executor#1", llm_provider=MockTeamProvider("ok"))
        team.members["executor#1"] = TeamMember(agent)
        team._members_by_name["executor#1"] = team.members["executor#1"]
        team.mailbox.register("executor#1")

        assert team.resolve_recipient("executor") == "executor#1"
    ...
    def test_resolve_bare_name_ambiguous_returns_none(self, tmp_path):
        team = _build_dynamic_team(tmp_path, {"executor": None})
        for n in (1, 2):
            agent = Agent(name=f"executor#{n}", llm_provider=MockTeamProvider("ok"))
            handle = f"executor#{n}"
    ...
    def test_live_instances_for_blueprint_sorted(self, tmp_path):
        ...
        assert team.live_instances_for_blueprint("executor") == [
            "executor#1",
            "executor#2",
            "executor#3",
        ]
...
class TestRosterManageTool:
    ...
    async def test_spawn_batch_returns_handles_only(self, tmp_path):
        ...
        try:
            result = await tool(
                action="spawn", members=["executor", "executor", "explorer"]
            )
            assert "executor#1" in result
            assert "executor#2" in result
    ...
    async def test_spawn_explicit_handle_reuses_history(self, tmp_path):
        ...
        await team.start()
        try:
            first = await tool(action="spawn", members=["executor#3"])
            assert "executor#3" in first
            await tool(action="dismiss", members=["executor#3"])
            second = await tool(action="spawn", members=["executor#3"])
            assert "executor#3" in second
            assert set(team.members) == {"executor#3"}
        finally:
            await team.stop()
    ...
    async def test_dismiss_batch_and_partial_success(self, tmp_path):
        ...
        await team.start()
        try:
            await tool(action="spawn", members=["executor", "executor"])
            result = await tool(action="dismiss", members=["executor#1", "executor#99"])
            assert "Dismissed: executor#1" in result
            assert "Not live: executor#99" in result
            assert set(team.members) == {"executor#2"}
        finally:
            await team.stop()
    ...
    async def test_dismiss_requires_explicit_handles(self, tmp_path):
        ...
        try:
            await tool(action="spawn", members=["executor", "executor"])
            result = await tool(action="dismiss", members=["executor"])
            assert "Use explicit handles" in result
            assert "executor#1" in result and "executor#2" in result
        finally:
            await team.stop()
...
class TestHandleHelpers:
    def test_parse_valid(self):
        assert parse_instance_handle("executor#1") == ("executor", 1)
        assert parse_instance_handle("alpha-beta#42") == ("alpha-beta", 42)

    def test_parse_invalid(self):
        assert parse_instance_handle("executor") is None
        assert parse_instance_handle("executor#abc") is None
        assert parse_instance_handle("executor#") is None
        assert parse_instance_handle("") is None

    def test_make(self):
        assert make_instance_handle("executor", 7) == "executor#7"
...
class TestAgentTeamConstructorBlueprints:
    ...
    async def test_blueprint_dataclass_holds_metadata(self, tmp_path):
        path = _write_md(tmp_path / "ex.md", name="executor", description="d")
        bp = MemberBlueprint(name="executor", description="d", source_path=path)
        assert bp.name == "executor"
    ...
    async def test_get_status_includes_blueprints(self, tmp_path):
        team = _build_dynamic_team(tmp_path, {"executor": {"description": "d"}})
        status = team.status()
        assert "blueprints" in status
        bp_names = [b["name"] for b in status["blueprints"]]
        assert "executor" in bp_names
...
class TestEndToEndSmoke:
    async def test_spawn_message_dismiss_via_real_team(self, tmp_path):
        ...
        try:
            ...
            status = team.status()
            assert any(m["name"] == "executor#1" for m in status["members"])
    ...
    try:
        ...
        async with _session_factory()() as db:
            db.add(
                ChatSession(
                    parent_session_id=lead_uuid,
                    agent_name="executor#7",
                    title="executor#7",
                )
            )
            await db.commit()

        m = await team.spawn("executor")  # should auto-pick #8
        assert m.name == "executor#8"
        ...
        async with _session_factory()() as db:
            ...
            assert "executor#7" in child_names
            assert "executor#8" in child_names
    finally:
        await team.stop()
